...
class BlockchainLab {
  constructor() {
    ...
    const difficultyLeading = parseInt(process.env.DIFFICULTY_LEADING_ZEROS || '3', 10);
  ...
  createSession(adminToken) {
    ...
    const blockchain = {
      chain: [this.createGenesisBlock()], // Main canonical chain
      ...
      difficulty: this.calculateDifficulty(
        this.adminSettings.difficultyLeading,
        this.adminSettings.difficultySecondary
      ),
      createdAt: Date.now(),
      networkStats: {
        totalHashrate: 0,
  ...
  createGenesisBlock() {
    const genesisBlock = {
      ...
      transactions: [],
      miner: 'system',
      difficulty: this.adminSettings.currentDifficulty
    };
  ...
  addBlockToChain(sessionId, block) {
    ...
    if (participant) {
      ...
      participant.balance += this.adminSettings.miningRewardCoins;
    }
   ...
   */
  registerBlock(blockchain, block) {
  ...
  updateAdminSettings(adminToken, newSettings) {
    ...
    if (this.adminSettings.parametersLocked) {
      return { success: false, error: 'Parameters are locked' };
    }

    // Update settings
    if (newSettings.difficultyLeading !== undefined) {
      this.adminSettings.difficultyLeading = newSettings.difficultyLeading;
    }
    if (newSettings.difficultySecondary !== undefined) {
      this.adminSettings.difficultySecondary = newSettings.difficultySecondary;
    }
    if (newSettings.miningRewardCoins !== undefined) {
      this.adminSettings.miningRewardCoins = newSettings.miningRewardCoins;
    }
    if (newSettings.networkMode !== undefined) {
      this.adminSettings.networkMode = newSettings.networkMode;
    }

    // Recalculate difficulty
    this.adminSettings.currentDifficulty = this.calculateDifficulty(
      this.adminSettings.difficultyLeading,
      this.adminSettings.difficultySecondary
    );

    return { success: true, settings: this.adminSettings };
  }
