...
module.exports = function(io, app) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('block-found', async (data) => {
      ...
      const registerResult = app.blockchainLab.registerBlock(sessionId, blockchain, block);

      // 3. NETWORK PROPAGATION: Send ONLY to assigned peers (simulated P2P)
      const networkMode = session.adminSettings?.networkMode || 'simulated-p2p';
    ...
    socket.on('gossip-forward', async (data) => {
      ...
      if (session.adminSettings.networkMode === 'simulated-p2p') {
        const myPeers = session.peerAssignments?.get(userId) || [];
        myPeers.forEach(peerId => {
          io.to(`user-${peerId}`).emit('peer-block-received', { block, minerId });
        });
    ...
    socket.on('start-team-attack', async (data) => {
      const { sessionId, blocksBack: blocksBackRaw, modifiedCode } = data;
      if (socket.data?.role !== 'admin' || socket.data?.sessionId !== sessionId) {
        return socket.emit('error', 'Only the session admin can start this attack');
      }
      ...
      const blocksBack = Math.max(1, parseInt(blocksBackRaw, 10) || 2);

      // Only miners can extend a secret fork; wallets must not consume "colluder" slots
      const minerIds = Array.from(session.participants.entries())
        .filter(([, info]) => info && info.role === 'miner')
        .map(([id]) => id);

      if (minerIds.length < 2) {
        return socket.emit('error', 'Team attack needs at least 2 miners in the session (wallets do not count)');
      }

      minerIds.sort(() => 0.5 - Math.random());
      ...
      const forkBlock = chain[forkParentIndex];

      const currentHeight = tipIndex;
      const appliesAtBlock = currentHeight + blocksBack;

      const attackCode = modifiedCode || '// Attack Validator\nreturn { _broken: true };';

      session.teams = { colluders: new Set(colluders), honest: new Set(honest) };

      colluders.forEach(id => {
        const p = session.blockchain.participants.get(id);
        if (p) p.isAttacker = true;
      });
      honest.forEach(id => {
        const p = session.blockchain.participants.get(id);
        if (p) p.isAttacker = false;
      });

      // Tell all miners where the fork starts (clients set isColluding + collusionTipHash)
      io.to(`miner-${sessionId}`).emit('team-attack-started', {
        colluders,
        honest,
        forkBlock: { hash: forkBlock.hash, index: forkBlock.index },
        blocksBack
      });
      ...
      io.to(`admin-${sessionId}`).emit('attack-started', {
        colluders,
        honest,
        appliesAtBlock,
        forkFromIndex: forkBlock.index,
        forkFromHash: forkBlock.hash.substring(0, 16),
        blocksBack
      });
      if (DEBUG_MODE) {
        console.log(`51% attack started in session ${sessionId}. Colluders: ${colluders.join(', ')}. Fork parent block #${forkBlock.index}, applies at block ${appliesAtBlock}`);
      }
    });
    ...
    socket.on('propose-hard-fork', (data) => {
      ...
      if (socket.data?.role !== 'admin' || socket.data?.sessionId !== sessionId) {
        return socket.emit('error', 'Only the session admin can propose a hard fork');
      }
      ...
      if (!Number.isFinite(h) || h < 1) {
        return socket.emit('error', 'Invalid fork activation height');
      }
      
      session.pendingFork = { height: h, name: name || 'Hard Fork' };
      io.to(`session-${sessionId}`).emit('hard-fork-proposed', { height: h, name: session.pendingFork.name });
    });
    
    // User declares themselves an attacker (e.g. Double Spend)
    socket.on('declare-attacker', (data) => {
      const { sessionId } = data;
      const userId = socket.data?.userId;
      if (!userId) return;
      ...
      const session = app.blockchainLab.sessions.get(sessionId);
      if (session) {
        const p = session.blockchain.participants.get(userId);
        if (p) p.isAttacker = true;
        
        if (!session.teams) session.teams = { colluders: new Set(), honest: new Set() };
        session.teams.colluders.add(userId);
        session.teams.honest.delete(userId);
        
        // Trigger a state reload on all clients so the label shows up immediately
        io.to(`session-${sessionId}`).emit('node-joined', { userId: userId, timestamp: Date.now() });
      }
    });
