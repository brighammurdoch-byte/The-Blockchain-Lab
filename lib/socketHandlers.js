...
module.exports = function(io, app) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('process-peer-block', async (data) => {
      ...
      if (DEBUG_MODE) console.log(`Processing peer block for ${userId}: ${block.hash.substring(0, 16)}...`);
      
      try {
        // Update the specific participant's personal chain.
    ...
    // Server only broadcasts, does not validate
